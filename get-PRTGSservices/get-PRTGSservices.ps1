...
.DESCRIPTION
    ...
    PRTG Sensor settings;
        Example;
        -Computer %host -exclude gpsvc,IntelAudioService,"Intel(R) TPM Provisioning Service"
    Security Context: Use Windows credentials of parent device

    Sensor runs on probe server with Powershell 5.
    Eecommended scanning Interval - 5+ minutes.
...
.EXAMPLE
    .\get-PRTGSservices.ps1 -Computer server1.pihl.local -UptimeGrace 7200 -Exclude IntelAudioService -Verbose
...
try {
    ...
    $Output = [PSCustomObject]@{
        prtg = [PSCustomObject]@{
            ...
            [PSCustomObject]@{
                Channel = "Not started within reboot grace"
